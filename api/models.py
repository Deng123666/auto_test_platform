...
class User(AbstractUser):
    ...
    role = models.CharField(max_length=20, choices=ROLES, default='tester')
    phone = models.CharField(max_length=20, blank=True, null=True)
    department = models.CharField(max_length=100, blank=True, null=True)
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)

    # 添加自定义的 related_name 解决冲突
    groups = models.ManyToManyField(
        Group,
        verbose_name='groups',
        blank=True,
        help_text='The groups this user belongs to. A user will get all permissions granted to each of their groups.',
        related_name="api_user_groups",  # 添加这行
    ...
    user_permissions = models.ManyToManyField(
        ...
        related_name="api_user_permissions",  # 添加这行
        related_query_name="api_user",
    )
    ...
    class Meta:
        ...
        permissions = [
            ('manage_users', '可以管理用户'),
            ('manage_projects', '可以管理项目'),
            ('execute_tests', '可以执行测试'),
            ('view_reports', '可以查看报告'),
        ]
